...
set name "pyk2-cpd1-amber"
set fname "images/pyk2-allcpds-pathways.tga"
set fname2 "images/pyk2-allcpds-pathways-side.tga"
set pathways {"pyk2-cpd1-amber-pathway-4" "pyk2-cpd2-amber-pathway-15" "pyk2-cpd8-amber-pathway-2"}
...
set enmin 0.0
set enmax 20.0
set npathway [llength $pathways]
set colors [list "red" "blue" "green" "yellow" "magenta" "tan" ]
#for {set iarg 3} {$iarg < $narg} {incr iarg} {
#	set pathwayfname [lindex $argv $iarg]
for {set ipathway 0} {$ipathway < $npathway} {incr ipathway} {
	set pathwayfname [lindex $pathways $ipathway]
	mol new
	set input [open "$pathwayfname" "r"]
	while { [gets $input line] >= 0 } {
		scan $line "%d %g %g %g %g" i x y z en
		...
		if { $i > 1 } {
			...
			#	set cfrac 0.0
			#} else {
			#	set cfrac [expr ( $en - $enmin ) / ($enmax - $enmin) ]
			#}
			#set cidx [expr int( 1023 * $cfrac + 33) ]
			#draw color $cidx
			set c [lindex $colors $ipathway] 
			draw color $c
