...
def print_points(output,iter,interp,n,x,origin,spline):
	#print(x)
	etot=0.0
	en=interp(origin)[0]
	line='{0:5d} {1:4d} {2:6.3f} {3:6.3f} {4:6.3f} {5:6.3f}\n'.format(iter,1,origin[0],origin[1],origin[2],en)
	output.write(line)
	for i in range(0,n):
		en=interp(x[i,:])[0]
		etot+=en
		...
		if (spline is not None):
			...
			line='{0:5d} {1:4d} {2:12.6f} {3:12.6f} {4:12.6f} {5:6.3f} {6:6.3f} {7:6.3f} {8:6.3f}\n'.format(
				iter,i+2,x[i,0],x[i,1],x[i,2],en,l,mgrad,theta)
		else:
			line='{0:5d} {1:4d} {2:12.6f} {3:12.6f} {4:12.6f} {5:6.3f} {6:6.3f}\n'.format(
				iter,i+2,x[i,0],x[i,1],x[i,2],en,l)
		output.write(line)
		output.flush()
...
iter_interval=int(sys.argv[12])
outfname=sys.argv[13]
...
#print_points(sys.ointerp,npoints,init_coords,origin,None)
...
print_points(output,0,interp,npoints,points,origin,spline)
while (iter<=maxiter):
	...
	if ((iter%iter_interval)==0):
		print_points(output,iter,interp,npoints,points,origin,spline)
...
print_points(sys.stdout,iter,interp,npoints,final_coords,origin,spline)
sys.exit(0)
